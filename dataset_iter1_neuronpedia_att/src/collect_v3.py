#!/usr/bin/env python3
"""Optimized Neuronpedia attribution graph collector.

Confirmed rate limit: 30 requests per 60-minute sliding window.
Strategy:
- Poll until rate limit window opens
- Use 120s spacing for maximum sustained throughput (30/hour)
- Checkpoint after every single graph
- Round-robin across domains for balanced coverage
- Generate output files when done or on graceful exit
"""
...
logger.add(str(LOGS_DIR / "collect_v3.log"), rotation="30 MB", level="DEBUG")
...
SLUG_PREFIX = "m3"
REQUEST_SPACING = 120  # Exactly 30/hour (max throughput within limit)
CHECKPOINT_FILE = TEMP_DIR / "checkpoint_v3.json"
...
def _handle_signal(signum, frame):
    global _shutdown
    logger.warning(f"Signal {signum} received, finishing current graph then writing output...")
    _shutdown = True
...
def wait_for_rate_limit():
    """Poll the API until we get a non-429 response or the rate limit info shows remaining > 0."""
    logger.info("Checking rate limit status...")
    headers = {"Content-Type": "application/json", "x-api-key": API_KEY}

    while True:
        try:
            # Use a lightweight probe - send a request that will quickly tell us rate limit status
            # We use a dummy slug that will either succeed or 429
            resp = requests.post(API_URL, json={
                "prompt": "test",
                "modelId": "gemma-2-2b",
                "slug": f"{SLUG_PREFIX}-ratecheck-{int(time.time())}",
                "maxNLogits": 10,
            ...
            if resp.status_code == 429:
                ...
                logger.info(f"Rate limited: {remaining} remaining. Waiting 60s...")
                time.sleep(60)
                continue
            elif resp.status_code == 200:
                # Rate limit is open! This probe used 1 slot but generated a real graph
                logger.info("Rate limit open! First request succeeded.")
                return resp  # Return the successful response so we can use it
            else:
                logger.warning(f"Probe got HTTP {resp.status_code}, waiting 30s...")
                time.sleep(30)
        except Exception:
            logger.exception("Probe error, waiting 30s...")
            time.sleep(30)
...
def write_outputs(records: list[dict]) -> None:
    ...
    # Write full output - may need splitting if > 90MB
    out = WORKSPACE / "data_out.json"
    out.write_text(json.dumps(records, ensure_ascii=False))
    size_mb = out.stat().st_size / 1e6
    logger.info(f"Wrote data_out.json: {size_mb:.1f} MB")

    # If too large, split into parts
    if size_mb > 90:
        logger.info("File > 90MB, splitting into parts...")
        part_size = len(records) // 3 + 1
        for i in range(3):
            chunk = records[i*part_size:(i+1)*part_size]
            if chunk:
                pf = WORKSPACE / f"data_out_part{i+1}.json"
                pf.write_text(json.dumps(chunk, ensure_ascii=False))
                logger.info(f"  Wrote {pf.name}: {pf.stat().st_size/1e6:.1f} MB, {len(chunk)} records")
    ...
    for d in sorted(domain_recs):
        if domain_recs[d]:
            ...
            preview.append(r)
    (WORKSPACE / "data_out_preview.json").write_text(
...
def main():
    ...
    # Wait for rate limit to open
    logger.info("Waiting for rate limit window to open...")
    probe_resp = wait_for_rate_limit()

    # If probe succeeded, we wasted 1 rate-limited request on "test" prompt
    # That's OK - we used it to confirm the window is open
    logger.info("Rate limit window is open! Starting collection...")
    ...
    for work_idx, (domain, idx, prompt) in enumerate(remaining):
        ...
        if result is not None:
            ...
            if record is not None:
                ...
                logger.warning("  Graph too small, skipping")
                done_set.add(key)
                fail_count += 1
                save_checkpoint(records, list(done_set))
        else:
